...
async function run() {
  try {
    // 2. Clear old bookings and buses
    ...
    const { error: delBookingsError } = await supabase
      ...
      .neq('id', '0');
    if (delBookingsError) {
      console.error('Error deleting bookings:', delBookingsError);
    } else {
      console.log('Bookings cleared successfully.');
    }

    console.log('Clearing old buses...');
    const { error: delBusesError } = await supabase
      .from('buses')
      .delete()
      .neq('id', '0');
    if (delBusesError) {
      console.error('Error deleting buses:', delBusesError);
    } else {
      console.log('Buses cleared successfully.');
    }

    // 3. Seed Hacker Sessions
    console.log('Inserting new CyberStrike 2026 hacker sessions...');
    ...
    if (error) {
      console.error('Error seeding hacker sessions:', error);
      process.exit(1);
    }

    console.log('Successfully seeded database with hacker sessions:', data.length, 'records added.');
    process.exit(0);
  } catch (err) {
