...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const [seat, val] of Object.entries(rawAttendees)) {
      ...
      if (typeof val === 'object' && val !== null) {
        cleanAttendees[seat] = {
          ...
          vpName:         val.vpName || val.vp_name || val.vp || '',
          vpImage:        val.vpImage || val.vp_image || val.image || '',
          designation:    val.designation || val.desig || '',
    ...
    const ticket = {
      ...
      bookerVpName:    bk.booker_vp_name || bk.vp_name || bk.vpName || '',
